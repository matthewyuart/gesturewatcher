...
- **claim arbitration is by claim TYPE, not presence.** a `knob` claim is the only interaction that consumes this hand's roll, and it leaves the wrist rotated when the pinch ends — so tilt suspends to the base while twisting and re-arms only after the wrist passes back through the deadzone (`tiltArmedRef`), otherwise releasing a knob flings the filter to wherever the wrist stopped. every other claim (chord card, bpm/shade bar) leaves roll free and is this hand's *natural* target, so tilt keeps running through them — suspending there would snap the filter to base and back on every chord-card grab. thumb-to-finger chord touches set no claim at all, so normal chord playing and tilt are fully simultaneous.
- a hand raised into frame already tilted responds immediately (absolute angle → filter, as asked); the re-arm gate applies only after a knob twist.
...
- `src/audio/SynthEngine.ts` — 2 lead voices (dual detuned osc, envelope-gated, melody uses voice 0) + poly chord pad → shared lowpass → feedback delay → limiter → analyser. **melody and chord waves are separate**: `setWave` (leads) and `setChordWave` (pad, tone sheet). chord tone `'pad'` = the original blend (saw on the bottom voice, triangle above) and is the default; the 4 plain waves apply uniformly, each with a level trim in `chordLevel()` so switching doesn't jump the mix. `setChordWave` retunes a sounding chord in place — no retrigger. the knobs (filter/res/attack/release/echo/volume) stay shared by leads + pad. **rejects non-finite frequencies** (guard against NaN from degenerate layout rects — removing this once caused a full react unmount crash).
